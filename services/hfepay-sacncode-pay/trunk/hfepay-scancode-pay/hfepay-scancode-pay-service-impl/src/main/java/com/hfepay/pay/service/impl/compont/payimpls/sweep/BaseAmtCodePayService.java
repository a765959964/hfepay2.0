 ...
 */
@Component
public class BaseAmtCodePayService extends BasePayService {
	public static final Logger log = LoggerFactory.getLogger(BaseAmtCodePayService.class);
	...
	//限额在前台已经做了，此处不需要再次做限额，检查参数即可
	@Override
	protected void checkParams(OrderBo orderBo) throws PayException {
		super.checkParams(orderBo);
		if(null==orderBo.getPrice()||orderBo.getPrice().compareTo(new BigDecimal("0"))<=0
				||null==orderBo.getDiscountAmt()||orderBo.getDiscountAmt().compareTo(new BigDecimal("0"))<0){
			log.error("#######sweep pay price,DiscountAmt is null######");
