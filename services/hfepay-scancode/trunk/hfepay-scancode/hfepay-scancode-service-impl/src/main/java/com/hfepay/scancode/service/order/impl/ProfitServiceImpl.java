...
import com.hfepay.scancode.service.operator.AgentPaywayBakService;
...
public class ProfitServiceImpl implements ProfitService {
	...
	private void initStatics(String date) throws Exception{
		//计算交易记录，同时建数据插入临时表
		List<OrderPayStaticDTO> list = orderPayService.getSumOrderAmt(date,"02");//此处数据量太大的情况暂时没有考虑，主要是分页去统计数据不好操作,02交易01提现
		//临时表List用于批量插入
		...
		for (OrderPayStaticDTO orderPayStaticDTO : list) {//统计费率数据
			...
			if(null==rateList||list.isEmpty()){
				...
				if(listSettleTemp.size()==BATCHSIZE){
					//批量插入数据到临时表
					tempProfitService.insertBatch(listSettleTemp);
					profitDetailService.inserBatch(listSettleDetail);
				...
				for (Object rateObject : rateList) {//根据费率差计算分级利润
					...
					if(listTemp.size()==BATCHSIZE){
						...
						profitDetailService.inserBatch(listDetail);
						listTemp.clear();
						listDetail.clear();
					}
				}
	...
	public void doSaveWithdrawProfit(String date) throws Exception {
		//计算交易记录，同时建数据插入临时表
		List<OrderPayStaticDTO> list = orderPayService.getSumOrderAmt(date,"01");//此处数据量太大的情况暂时没有考虑，主要是分页去统计数据不好操作提现操作01，交易02
		//临时表List用于批量插入
		List<TempProfit> listTemp = new ArrayList<>();
		//分润明细表，临时表的扩充，同时是持久性数据
		List<ProfitDetail> listDetail = new ArrayList<>();
	...
	private void doSaveWithDrawTimesProfits(String date) throws Exception {
		// TODO Auto-generated method stub
		List<OrderPayStaticDTO> list = orderPayService.getWithDrawTimes(date,"01");//提现次数分成计算
		//临时表List用于批量插入
		List<TempProfit> listTemp = new ArrayList<>();
		//分润明细表，临时表的扩充，同时是持久性数据
		List<ProfitDetail> listDetail = new ArrayList<>();
