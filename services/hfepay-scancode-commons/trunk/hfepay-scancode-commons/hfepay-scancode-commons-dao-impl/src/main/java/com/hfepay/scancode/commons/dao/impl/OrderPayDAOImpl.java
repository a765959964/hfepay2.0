...
public class OrderPayDAOImpl extends MybatisEntityDAO<OrderPay, String> implements OrderPayDAO {
	...
	@Override
	public List<OrderPay> getTotalMoney(String date,String cashierNo,String identityFlag) {
		...
		return this.getSqlSession().selectList(sqlId("getTotalMoney"), map);
	}
	...
	@Override
	public BigDecimal getTotalInMoney(String date, String identityNo, String identityFlag){
		Map<String,String> map = new HashMap<>();
		map.put("date", date);
		if(identityFlag.equals("4")){
			map.put("cashierNo", identityNo);
		...
		return this.getSqlSession().selectOne(sqlId("getTotalInMoney"), map);
