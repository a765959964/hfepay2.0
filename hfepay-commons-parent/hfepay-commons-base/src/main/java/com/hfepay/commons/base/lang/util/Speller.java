...
import com.hfepay.commons.base.lang.Encoding;
...
public class Speller {
	 ...
	 * 要转成拼音的汉字
	 */
	private String inputChinese;	
	
	public static Speller of(String inputChinese) {
		return new Speller(inputChinese);
	}
	
	Speller(String inputChinese) {
	 ...
	 * @return
	 */
	private int toAscii(String chs) {
		int asc = 0;
		try {
			byte[] bytes = chs.getBytes(Encoding.GB2312);
			if (bytes == null || bytes.length > 2 || bytes.length <= 0) {
				throw Throwables.makeThrow("illegal resource string");				
			}
			if (bytes.length == 1) {
				asc = bytes[0];
			}
	 ...
	 * @return
	 */
	public String spelling(String sep) {
		String key, value;
		StringBuilder buffer = new StringBuilder();
		for (int i = 0; i < inputChinese.length(); i++) {
			key = inputChinese.substring(i, i + 1);
			...
			if (i > 0)
			buffer.append(Strings.defaultString(sep));
			buffer.append(value);
		}
	...
	/**
	 * 取得词组拼音前缀 主要用于取得中文姓名前缀，不是中文将跳过
	 * 
	 * @param chs
	 * @return
	 */
	public String spellingPrefix() {
		StringBuilder sb = new StringBuilder();
		for (int j = 0; j < this.inputChinese.length(); j++) {
			String key = this.inputChinese.substring(j, j + 1);
			if (key.getBytes().length >= 2) {
				String result = null;
				int ascii = toAscii(key);
				if (ascii > 0 && ascii < 160) {
					result = String.valueOf((char) ascii);
					sb.append(result);
				} else {
	...
	public static void main(String[] args) { 
		Sys.println(Speller.of("重庆").spelling(" "));
		Sys.println(Speller.of("狂喜猛增瑛").spellingPrefix());
		Sys.println(Speller.of("狂喜猛增瑛").toAscii("狂"));
		char c = '狂';
		Sys.println(Integer.toHexString(c&0xff));
