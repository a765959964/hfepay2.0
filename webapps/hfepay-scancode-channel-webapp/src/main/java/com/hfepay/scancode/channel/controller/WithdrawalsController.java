...
public class WithdrawalsController extends BaseController{
	...
	public String listContent(HttpServletRequest request,ModelMap model,WithdrawalsCondition withdrawalsCondition) throws Exception{	
		...
		if(Constants.IDENTITYFLAG_CHANNEL.equals(user.getIdentityFlag())){
			withdrawalsCondition.setChannelNo(user.getChannelCode());
		}else if(Constants.IDENTITYFLAG_AGENT.equals(user.getIdentityFlag())){
			withdrawalsCondition.setAgentNo(user.getAgentNo());
		}else if(Constants.IDENTITYFLAG_MERCHANT.equals(user.getIdentityFlag())){
			withdrawalsCondition.setMerchantNo(user.getMerchantNo());
		}
