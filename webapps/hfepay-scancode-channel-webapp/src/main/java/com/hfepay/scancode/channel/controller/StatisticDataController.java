...
public class StatisticDataController extends ScanBaseController {
	@Autowired
	...
	public JSON orderStatistic(HttpServletRequest request, MerchantOrderStatisticCondition condition) {
		...
		ChannelAdmin  admin = getCurrentUser();
		condition.setChannelNo(admin.getChannelCode());
		if(Strings.isEmpty(condition.getAgentNo())){
			condition.setAgentNo(admin.getAgentNo());
		}
		if(Strings.isEmpty(condition.getMerchantNo())){
	...
	public void counteExcel(HttpServletRequest request, HttpServletResponse response,
		...
		//表头
		 List<String> columns = Arrays.asList(new String[]{"时间","支付宝支付成功成功笔数","支付宝支付成功失败笔数","微信支付成功笔数","微信支付失败笔数"});
		//参数列表数据
	...
	public void amtExcel(HttpServletRequest request, HttpServletResponse response,
		...
		//表头
		 List<String> columns = Arrays.asList(new String[]{"时间","支付宝支付成功金额","支付宝支付失败金额","微信支付成功金额","微信支付失败金额"});
		//参数列表数据
		 List<String> props = columns;
		String title = null;// apiName +"详情统计"+Strings.getUUID();
