...
function updateStatus(btn){
	...
	var merchantNo = $this.attr("merchantNo");
	bootbox.setLocale("zh_CN");  //设置按钮为中文
    bootbox.confirm("确定执行该操作吗?", function(result) {
  	  if(result){
  		$.ajax({
 		   type: "POST",
 		   url: "merchantactivity/updateStatus",
 		   data:{"id":obj,"status":status,"merchantNo":merchantNo},
...
function del(obj,activityId){
    ...
    bootbox.confirm("确定执行该操作吗?", function(result) {
  	  if(result){
  		$.ajax({
 		   type: "POST",
 		   url: "merchantactivity/del",
 		   data:{"id":obj,"activityId":activityId},
 		   success: function(msg){
 			  msg = $.parseJSON(msg);
 			 bootbox.alert(msg.values,function(){
				 if(msg.executeStatus == 0){
...
function toPage(pageNumber){
	...
	$.ajax({
		   type: "POST",
		   url: "merchantactivity/content?pageNo="+pageNumber,
		   data:datajson,
		   success: function(msg){
			     $("#tablec").html(msg);
		   }
