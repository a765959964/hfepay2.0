...
<table class="table table-striped table-bordered table-hover" id="sample_6">
						  ...
						  <c:forEach var="item" items="${pager.result}"> 
						   <tr>
									...
									<td class="parent_id_parse" title="rate"><fmt:formatNumber value="${item.rate}" pattern="#0.##"/></td>
									<c:if test="${item.status=='1'}">
										<td class="parent_id_parse">有效</td>
									</c:if>
									<c:if test="${item.status=='2'}">
										<td class="parent_id_parse">无效</td>
									</c:if>
							...
							<td>
								...
								<c:if test="${item.agentNo != user.agentNo}">
