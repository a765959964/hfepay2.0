							...
							<td>
								...
								<%-- <c:if test="${not empty item.qrCode}"><a href="${path}/adminManage/qrcodegoods/edit/0/${item.merchantNo}/${item.qrCode}"><span class="label label-success">绑定商品</span></a></c:if> --%>
						   		...
						   		<c:if test="${user.identityFlag=='1'}">
						   			<c:if test="${not empty item.qrCode && (item.status == 0 || item.status == 1)}"><a onclick="javascript:merReview('${item.merchantNo}','${item.id}','3','4');" value="${item.id}"><span class="label label-success">审核</span></a></c:if>
						   		</c:if>
						   		<c:if test="${user.identityFlag=='2'}">
						   			<c:if test="${not empty item.qrCode && item.status == 0}"><a onclick="javascript:merReview('${item.merchantNo}','${item.id}','1','2');" value="${item.id}"><span class="label label-success">审核</span></a></c:if>
						   		</c:if>
...
<%-- 								<c:if test="${item.status == 3}"><a onclick="javascript:pushMerchant('${item.merchantNo}');"><span class="label label-primary">通知商户</span></a></c:if>	 --%>
								...
								<c:if test="${user.identityFlag=='1'}"><a onclick="javascript:del('${item.id}');" ><span class="label label-danger">删除</span></a></c:if>
							</td> 
						 </tr>
					</c:forEach>
			...
			<script type="text/javascript">
						...
						//审核
						function merReview(merchantNo,id,pass,reject){
							var flag = 0;
							var errorMsg = ""; 
							$.ajax({
								   type: "POST",
						...
						function updDetail(id,status){
							...
							$.ajax({
						 		   ...
						 		   success: function(msg){
						 			  msg = $.parseJSON(msg);
						 			  bootbox.alert(JSON.stringify(msg.values).replace(/"/g,''),function(){
