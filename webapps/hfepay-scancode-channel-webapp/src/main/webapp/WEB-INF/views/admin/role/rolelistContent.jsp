...
<%-- 								<a onclick="javascript:del('${item.id}');"><span class="label label-danger">删除</span></a> --%>
					...
					<c:if test="${roleList.result.size() == 0}">
								<tr>
									<td class="page-nodata" colspan="5">查无数据！</td>
								</tr>
					</c:if>
			</tbody>
			</table>
				    		...
				    		success:{   
						       ...
						       callback: function() {
						    	    var description = $('#description').val().trim();
						    	   	...
						    	   	if(description == '' || roleName == '') flag++;
						    	   	if(flag > 0){
						    	   		bootbox.alert("角色名称与描述不可为空！",function(){});
						    	   	}else{
							    	    if(id != null && id != ''){
							    	    	$.ajax({
												   type: "POST",
			...
			<page:page curPage="${roleList.pageNo }" totalPages="${roleList.totalPages }"  totalCounts="${roleList.totalCount }"/>		
