				...
				<div class="col-md-12">
					...
					<div class="portlet box green-haze">
						...
						<div class="portlet-body" id="tablec">
							...
							<tbody>
								  <c:forEach var="rootMenu" items="${rootMenu1}" varStatus="statusroot">
								  <tr>
									<td>
										<input type="checkbox" id="root${rootMenu.id }" value="${rootMenu.id }" data="${rootMenu.hasPerssion }"/><label for="root${rootMenu.id }">${rootMenu.menuName }</label><!-- 一级菜单菜单名称 -->
									</td>
									<td>
										<table id="secondResource${rootMenu.id }"><!-- 二级菜单 -->
											<tbody>
											 	<c:forEach var="second" items="${secondMenu1}" varStatus="statusSecond">
											 	 	<c:if test="${rootMenu.id==second.key}">
											 	 		<c:forEach var="secondMenu" items="${second.value}" varStatus="statusSs"> 
													 		<tr>
																<td>
																	<input type="checkbox" id="second${secondMenu.id }" value="${secondMenu.id }" data="${secondMenu.hasPerssion }"/><label for="second${secondMenu.id }">${secondMenu.menuName }</label><!-- 二级菜单菜单名称 -->
																</td>
																<td>
																	<table id="thirdResource${secondMenu.id }"><!-- 三级菜单 -->
																		<tbody>
																		 	<c:forEach var="third" items="${thirdMap1}" varStatus="statusthird">
																		 	 	<c:if test="${secondMenu.id==third.key}">
