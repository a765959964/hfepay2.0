$(function(){
	...
	$(".ajaxCheck").blur(function(){
		...
		var tel =  /^[A-Za-z0-9_]{1,32}$/g;
		if(!tel.test(val)){
			flag++;
			errMsg = '需是数字、字母、下划线且长度不可超过32的组合';
		}
//		tel =  /[@#\$%\^&\*]+/g;
//		if(tel.test(val)){
//			flag++;
   	   ...
   	    rules: {
   	    ...
   	    submitHandler:function(form){$("#save").attr("disabled","disabled");
   			...
   			if(flag == 0){
   				...
   				$.ajax({
   					url:baseUrl+"/adminManage/channelbase/savechanneladmin",
   					data:datajson,
   					type:"POST",
   					success:function(msg)
   					{
