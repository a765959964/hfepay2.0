...
<table class="table table-striped table-bordered" id="sample_6">
	...
	<tr>		
		...
		<td class="col-md-2 td0">店内图片
		<c:if test="${not empty item.storeImg}">
			<br><br><a style="cursor:pointer;" class="refreshImg" attr="/${item.merchantNo}/storeImg">刷新图片</a>
		</c:if>
		</td>
		<td class="col-md-4 parent_id_parse" title="storeImg">
		<c:if test="${not empty item.storeImg}">
		<div class="small_pic">
			<a href="#pic_storeImg">
	...
	<tr>
		<td class="col-md-2 td0">门店图片
			<c:if test="${not empty item.storeImg}">
				<br><br><a style="cursor:pointer;" class="refreshImg" attr="/${item.merchantNo}/storePhotosImg">刷新图片</a>
			</c:if>
		</td>
		<td class="col-md-4 parent_id_parse" colspan="3" title="storePhotosImg">
	...
	<tr>
		<td class="col-md-2 td0">操作人名称</td><td class="col-md-4" title="${item.operator}">${item.operator}</td>
