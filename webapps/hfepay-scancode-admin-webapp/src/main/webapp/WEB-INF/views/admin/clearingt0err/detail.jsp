...
<table class="table table-striped table-bordered" id="sample_6">
	...
	<tr>
		...
		<td class="col-md-2 td0">订单状态</td>
		<td class="col-md-4 parent_id_parse" title='resultCode'>
<!-- 		//0:订单已创建 1:处理中2:处理成功 3:处理失败 -->
		<c:if test="${item.resultCode=='0' }">
		订单已创建
		</c:if>
		<c:if test="${item.resultCode=='1' }">
		处理中
		</c:if>
		<c:if test="${item.resultCode=='2' }">
		处理成功
		</c:if>
		<c:if test="${item.resultCode=='3' }">
		处理失败
		</c:if>
		</td>
	</tr>
