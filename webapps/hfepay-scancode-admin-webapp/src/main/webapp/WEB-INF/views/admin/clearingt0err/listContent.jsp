...
<table class="table table-striped table-bordered table-hover" id="sample_6">
						  ...
						  <c:forEach var="item" items="${pager.result}"> 
						   <tr>
									...
									<td>${item.transAmt}</td>
									<td>${item.arrivalAmt}</td>
									<td align="center">
									<c:if test="${item.checkFlag == '01'}">
										对方有本地无
									</c:if>
									<c:if test="${item.checkFlag == '10'}">
										<span style="color: #f00">本地有对方无</span>
									</c:if>
									<c:if test="${item.checkFlag == '11'}">
										双方不一致
									</c:if>
									</td>
									<td>
									<c:if test="${item.processingStatus == '0'}">
										未处理
									</c:if>
									<c:if test="${item.processingStatus == '1'}">
										处理成功
									</c:if>	
									<c:if test="${item.processingStatus == '2'}">
										处理失败
									</c:if>
									</td>
