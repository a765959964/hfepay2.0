						  ...
						  <c:forEach var="item" items="${roleList.result}" varStatus="status"> 
						   ...
						   <td>
							...
							<td>
								...
								<c:if test="${item.roleLevel=='2'}">
									代理商
								</c:if>
								<c:if test="${item.roleLevel=='3'}">
									商户
								</c:if>
							</td>
							...
							<td>
								<a href="${path }/adminManage/channel/role/roleList/roleEdit?id=${item.id}"><span class="label label-primary">修改权限</span></a>
...
<%-- 								<a onclick="javascript:del('${item.id}');"><span class="label label-danger">删除</span></a> --%>
			...
			<script>
				...
				$(".editRole").on("click",function(){
					...
					var channelCode = updBtn.parent().parent().find("td:eq(3)").attr("channelNo");
			...
			<page:page curPage="${roleList.pageNo}" totalPages="${roleList.totalPages}"/>	<b class="totalCountInPage">共&nbsp;&nbsp;${roleList.totalCount}&nbsp;&nbsp;条				
