					...
					<form class="form-horizontal" id="form">
							<div class="form-body">
								...
								<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											<label class="control-label col-md-2">支付通道
											</label>
											<div class="col-md-4">
												<select  id="payName" class="form-control" name="payName" <c:if test="${Obj != null}"> disabled="disabled" </c:if>>
														<option value="" selected = "selected">----请选择----</option>
															<c:forEach var="item" items="${mappingDicionList}">
																<c:if test="${Obj == null}">
																	<option value = "${item.payName}" payCode="${item.payCode}">${item.payName}</option>
																</c:if>
																<c:if test="${Obj != null}">
																	<option value = "${item.payName}" <c:if test="${item.payCode==Obj.payCode}">selected="selected"</c:if> payCode="${item.payCode}">${item.payName}</option>
																</c:if>
															</c:forEach>
													</select>
											</div>
										...
										<div class="form-group">
											...
											<div class="col-md-4">
												<input type="text" class="form-control ajaxCheck" id="rate"
													name="rate" value="<fmt:formatNumber value="${Obj.rate}" pattern="#0.##"/>" placeholder="提现手续费(元)">
											</div>
										</div>
									</div>
								</div>								
								<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											<label class="control-label col-md-2">清算手续费(元)</label>
											<div class="col-md-4">
												<input type="text" class="form-control ajaxCheck" id="rateAmount"
													name="rateAmount" value="<fmt:formatNumber value="${Obj.rateAmount}" pattern="#0.##"/>" placeholder="清算手续费(元)">
											</div>
										</div>
									</div>
								</div>
